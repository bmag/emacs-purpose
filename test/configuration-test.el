...
hash tables for the uncompiled alists."
  (purpose-with-empty-config
    (let
      ...
      (should (purpose-set-equal-p (hash-table-keys purpose--user-mode-purposes) '(dired-mode prog-mode)))
      (should (purpose-set-equal-p (hash-table-values purpose--user-mode-purposes) '(dired edit)))
      (should (purpose-set-equal-p (hash-table-keys purpose--user-name-purposes) '("foo" "editor")))
      (should (purpose-set-equal-p (hash-table-values purpose--user-name-purposes) '(bar edit)))
      (should (purpose-set-equal-p (hash-table-keys purpose--user-regexp-purposes) '("^\\*")))
      (should (purpose-set-equal-p (hash-table-values purpose--user-regexp-purposes) '(common))))
...
correct hash tables."
  (purpose-with-empty-config
    ...
    (should (purpose-set-equal-p (hash-table-keys purpose--default-mode-purposes)
                                 '(package-menu-mode
                                   image-mode
                                   compilation-mode
                                   grep-mode
    ...
    (should (purpose-set-equal-p (hash-table-values purpose--default-name-purposes)
                                 '(terminal edit edit)))
    (should (purpose-set-equal-p (hash-table-keys purpose--default-regexp-purposes)
                                 '("^ \\*Minibuf-[0-9]*\\*$")))
    (should (purpose-set-equal-p (hash-table-values purpose--default-regexp-purposes)
                                 '(minibuf)))))
...
This one tests `purpose-with-temp-purposes'."
  (let ((original-purposes (purpose-get-all-purposes)))
    (purpose-with-temp-purposes
        '(("foo" . foo)) '((".*\\.c" . c)) '((python-mode . py))
      (should (purpose-set-equal-p (purpose-get-all-purposes) '(c foo general py))))
    (should (purpose-set-equal-p (purpose-get-all-purposes) original-purposes))))
...
This one tests `purpose-with-empty-purposes'."
  (let ((original-purposes (purpose-get-all-purposes)))
    (purpose-with-empty-purposes
      (should (purpose-set-equal-p (purpose-get-all-purposes) '(general))))
    (should (purpose-set-equal-p (purpose-get-all-purposes) original-purposes))))
...
This one tests `purpose-with-additional-purposes'."
  (let ((original-purposes (purpose-get-all-purposes)))
    (purpose-with-additional-purposes
        '(("foo" . foo)) '((".*\\.c" . c)) '((python-mode . py))
      (should (purpose-set-equal-p (purpose-get-all-purposes) (delete-dups (append original-purposes '(c foo py))))))
    (should (purpose-set-equal-p (purpose-get-all-purposes) original-purposes))))
...
(ert-deftest purpose-test-ext-convenience ()
  ...
  (purpose-with-empty-purposes
    ...
    (should (purpose-set-equal-p (hash-table-keys purpose--extended-name-purposes) nil))
    (should (purpose-set-equal-p (hash-table-values purpose--extended-name-purposes) nil))
    (should (purpose-set-equal-p (hash-table-keys purpose--extended-regexp-purposes) nil))
    (should (purpose-set-equal-p (hash-table-values purpose--extended-regexp-purposes) nil))
    ...
    (should (purpose-set-equal-p (hash-table-keys purpose--extended-name-purposes) nil))
    (should (purpose-set-equal-p (hash-table-values purpose--extended-name-purposes) nil))
    (should (purpose-set-equal-p (hash-table-keys purpose--extended-regexp-purposes) '("\\.hy$")))
    (should (purpose-set-equal-p (hash-table-values purpose--extended-regexp-purposes) '(python)))

    (message "Testing purpose-remove-extension-purposes ...")
    (purpose-remove-extension-purposes :python
                                       :modes '(inferior-python-mode)
                                       :regexps '("\\.hy$"))
    (should (purpose-set-equal-p (hash-table-keys purpose--extended-mode-purposes) '(python-mode)))
    (should (purpose-set-equal-p (hash-table-values purpose--extended-mode-purposes) '(python)))
    (should (purpose-set-equal-p (hash-table-keys purpose--extended-name-purposes) nil))
    (should (purpose-set-equal-p (hash-table-values purpose--extended-name-purposes) nil))
    (should (purpose-set-equal-p (hash-table-keys purpose--extended-regexp-purposes) nil))
    (should (purpose-set-equal-p (hash-table-values purpose--extended-regexp-purposes) nil))))
...
(ert-deftest purpose-test-user-convenience ()
  ...
  (let ((original-purposes (purpose-get-all-purposes)))
    ...
    (purpose-add-user-purposes :modes '((org-mode . org)
                               ...
                               :regexps '(("^\\*foo" . foo)))
    (should (purpose-set-equal-p (purpose-get-all-purposes)
